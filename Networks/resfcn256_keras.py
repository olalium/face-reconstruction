...
class resfcn256_keras(object):
    ...
    def get_keras_model(self):
        ...
        #decoder
        x_d = Conv2DTranspose(self.base_filters * 32, 4, strides = (1,1), padding = 'same', activation = keras_backend.relu)(x_e) # 8 x 8 x 512

        x_d = Conv2DTranspose(self.base_filters * 16, 4, strides = (2,2), padding = 'same', activation = keras_backend.relu)(x_d) # 16 x 16 x 256
        x_d = Conv2DTranspose(self.base_filters * 16, 4, strides = (1,1), padding = 'same', activation = keras_backend.relu)(x_d) # 16 x 16 x 256
        x_d = Conv2DTranspose(self.base_filters * 16, 4, strides = (1,1), padding = 'same', activation = keras_backend.relu)(x_d) # 16 x 16 x 256

        x_d = Conv2DTranspose(self.base_filters * 8, 4, strides = (2,2), padding = 'same', activation = keras_backend.relu)(x_d) # 32 x 32 x 128
        x_d = Conv2DTranspose(self.base_filters * 8, 4, strides = (1,1), padding = 'same', activation = keras_backend.relu)(x_d) # 32 x 32 x 128
        x_d = Conv2DTranspose(self.base_filters * 8, 4, strides = (1,1), padding = 'same', activation = keras_backend.relu)(x_d) # 32 x 32 x 128

        x_d = Conv2DTranspose(self.base_filters * 4, 4, strides = (2,2), padding = 'same', activation = keras_backend.relu)(x_d) # 64 x 64 x 64
        x_d = Conv2DTranspose(self.base_filters * 4, 4, strides = (1,1), padding = 'same', activation = keras_backend.relu)(x_d) # 64 x 64 x 64
        x_d = Conv2DTranspose(self.base_filters * 4, 4, strides = (1,1), padding = 'same', activation = keras_backend.relu)(x_d) # 64 x 64 x 64

        x_d = Conv2DTranspose(self.base_filters * 2, 4, strides = (2,2), padding = 'same', activation = keras_backend.relu)(x_d) # 128 x 128 x 32
        x_d = Conv2DTranspose(self.base_filters * 2, 4, strides = (1,1), padding = 'same', activation = keras_backend.relu)(x_d) # 128 x 128 x 32

        x_d = Conv2DTranspose(self.base_filters * 1, 4, strides = (2,2), padding = 'same', activation = keras_backend.relu)(x_d) # 256 x 256 x 16
        x_d = Conv2DTranspose(self.base_filters * 1, 4, strides = (1,1), padding = 'same', activation = keras_backend.relu)(x_d) # 256 x 256 x 16

        x_d = Conv2DTranspose(self.out_channels, 4, strides = (1,1), padding = 'same', activation = keras_backend.relu)(x_d)
        x_d = Conv2DTranspose(self.out_channels, 4, strides = (1,1), padding = 'same', activation = keras_backend.relu)(x_d)
        x_d = Conv2DTranspose(self.out_channels, 4, strides = (1,1), padding = 'same', activation = keras_backend.sigmoid)(x_d)
