...
from networks.predictor import PosPrediction_6, MobilenetPosPredictor
...
def main():
	...
	for face_folder in os.listdir(image_folder):
		if face_folder == 'results':
			continue
		front_img = os.path.join(image_folder, face_folder, 'front' + img_type)#'front.jpg')#face_folder + '_front.png')
	...
	for images in face_imgs:
		...
		#kpt_indices = np.concatenate((extra_face_ind,bfm_kpt_ind))
		#mesh = trimesh.Trimesh(vertices=all_vertices[kpt_indices, :])
		#mesh.export('kpt_bfm_vertices.ply')
		...
		#write_obj_with_colors(images[0].replace('.jpg', '.obj'), save_vertices, triangles, colors)
