...
from facegen_to_posmap import generate_posmap_facegen, generate_posmap_facegen_bfm
import os
...
sys.path.append('../../')
from PRNetTwoFace.dataset_reader.DatasetReader import DatasetReader
import face3d
...
def apply_random_background(image):
    ...
    if (bg_img.shape[0] < 256 or bg_img.shape[1] < 256): # should not happend according to texture dataset specifications
...
def generate_facegen_dataset(root_dataset_folder, save_dataset_folder, save_dataset_label):
    ...
    for i, face in enumerate(face_list):
        ...
        if os.path.exists(front_face_save_path) and os.path.exists(left_face_save_path) and os.path.exists(right_face_save_path) and os.path.exists(pos_map_path):
        ...
        # crop images
        front_image_cropped = skimage.transform.warp(front_image, front_img_cropping_tform.inverse, output_shape=(image_h, image_w), preserve_range=True)
        left_image_cropped = skimage.transform.warp(left_image, side_img_cropping_tform, output_shape=(image_h, image_w), preserve_range=True)
        right_image_cropped = skimage.transform.warp(right_image, side_img_cropping_tform, output_shape=(image_h, image_w), preserve_range=True)
