...
def main(args):
    ...
    for epoch in range(epochs):
        ...
        for iters in range(num_train_iterations):
            ...
            stdout.flush()
            stdout.write('\riters:%d/%d epoch:%d,loss:%f,accuracy:%f'%(iters + 1, num_train_iterations, epoch, metrics[0], metrics[1]))
        ...
        fp_log.writelines('[%s] epoch:%d learning rate:%f validation loss:%f, validation accuracy:%f, saved:%s\n'
                            %(datetime.now().strftime("%Y_%m_%d_%H_%M_%S"), epoch, learning_rate, val_metrics[0], val_metrics[1], model_saved))
...
if __name__ == '__main__':
    ...
    par.add_argument('--model_path',default='checkpoint/keras_mobilenet_prn.h5',type=str,help='The path of the trained model')
    par.add_argument('--gpu',default='0',type=str,help='The GPU ID')
    par.add_argument('--model', default='mobilenet', type=str, help='Choose which backbone network to train on, mobilenet or prnet')
    par.add_argument('--alpha_mobilenet', default=1.2, type=float, help='Set alpha value for the mobilenet')
    par.add_argument('--resume_model', default=False, type=bool, help='Set if the network should resume training on a pretrained network')
    par.add_argument('--resume_model_path', default='checkpoint/keras_mobilenet_prn_trained.h5', type=str, help='Model path of pretrained network')
